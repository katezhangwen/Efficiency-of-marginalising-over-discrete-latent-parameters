...
x <- as.numeric(unlist(read.table("tdata1.txt")))
...
r1 <- sampling("tdata1.txt", result, resultr)
```
...
sigma = sqrt(1/prec)
```{r data2 density}
x <- as.numeric(unlist(read.table("tdata2.txt")))
plot(density(x), main="data2")
```

```{r sampling with data2, echo=FALSE, include= FALSE}

r2 <- sampling("tdata2.txt", result, resultr)
```
...
sigma = sqrt(1/prec)
```{r data3 density}
x <- as.numeric(unlist(read.table("tdata3.txt")))
plot(density(x), main="data3")
```

```{r sampling with data3, echo=FALSE, include= FALSE}
r3 <- sampling("tdata3.txt", result, resultr)
```
...
sigma = sqrt(1/prec)
```{r data4 density}
x <- as.numeric(unlist(read.table("tdata4.txt")))
plot(density(x), main="data4")
```


```{r sampling with data4, echo=FALSE, include= FALSE}
r4 <- sampling("tdata4.txt", result, resultr)
```
...
```{r data5 density}
x <- as.numeric(unlist(read.table("tdata5.txt")))
plot(density(x), main="data5")
```


```{r sampling with data5, echo=FALSE, include= FALSE}
r5 <- sampling("tdata5.txt", result, resultr)
```
...
ggplot(m_n_all_data.df, aes(x=as.numeric(n_rnames), y=value, col=variable))+ geom_point()+
  ...
  scale_x_continuous(labels=c("m1","m1r","m3(r)","m2(r)"),
                     breaks=c(1,2,3,4))+
  theme(legend.position = "right",legend.title = element_blank(),
        panel.grid.minor = element_blank())
